;;; packages.el --- Install and configure scimax packages
;;; Commentary:
;;
;; This is a starter kit for scimax. This package provides a
;; customized setup for emacs that we use daily for scientific
;; programming and publication.
;;
;; see https://github.com/jwiegley/use-package for details on use-package


;;; Code:

(setq use-package-always-ensure t)

(defun scimax-package-excluded-p (package)
  "Return non-nil if PACKAGE symbol is in `scimax-excluded-packages'."
  (member package scimax-excluded-packages))

;; * org-mode
;; load this first before anything else to avoid mixed installations
(use-package org-plus-contrib
  :mode ("\\.org\\'" . org-mode)
  :init
  ;; Use the current window for C-c ' source editing
  (setq org-src-window-setup 'current-window
	org-support-shift-select t)

  ;; I like to press enter to follow a link. mouse clicks also work.
  (setq org-return-follows-link t)
  :bind
  (("C-c l" . org-store-link)
   ("C-c L" . org-insert-link-global)
   ("C-c o" . org-open-at-point-global)
   ("C-c a" . org-agenda)
   ("C-c c" . org-capture)
   ("s-<SPC>" . org-mark-ring-goto)
   ("H-." . org-time-stamp-inactive)))

;; [2019-01-07 Mon] This is another package I don't use, and that sometimes is a
;; problem on windows installations
;; (use-package org-bullets)

;; [2019-01-07 Mon] I don't use this now, and it frequently causes an issue on
;; installing scimax
;; (use-package org-edna
;;   :init (org-edna-load))


;; * Other packages
(use-package diminish)

(unless (scimax-package-excluded-p 'aggressive-indent)
  (use-package aggressive-indent
    :config (add-hook 'emacs-lisp-mode-hook #'aggressive-indent-mode)))

(unless (scimax-package-excluded-p 'auto-complete)
  (use-package auto-complete
    :diminish auto-complete-mode
    :config (ac-config-default)))

(unless (scimax-package-excluded-p 'avy)
  (use-package avy))


;; May 24, 2017: this seems to be causing emacs 25.2 to be crashing on my linux box.
(unless (or (scimax-package-excluded-p 'tex)
            (eq system-type 'gnu/linux))
  (use-package tex
    :ensure auctex))

;; [2019-01-07 Mon] This also sometimes causes problems installing scimax,
;; especially on Windows.
;; Make cursor more visible when you move a long distance
;; (use-package beacon
;;   :config
;;   (beacon-mode 1))


(unless (scimax-package-excluded-p 'bookmark)
  (use-package bookmark
    :init
    (setq bookmark-default-file (expand-file-name "user/bookmarks" scimax-dir)
	  bookmark-save-flag 1)))


(unless (scimax-package-excluded-p 'bookmark+)
  (use-package bookmark+
    ;; I am not currently using this, and it loads a bunch of files on startup.
    :disabled t))

(unless (scimax-package-excluded-p 'button-lock)
  (use-package button-lock))

;; Potential for commandline scripts using emacs
(unless (scimax-package-excluded-p 'commander)
  (use-package commander
    :disabled t))

(unless (scimax-package-excluded-p 'drag-stuff)
  (use-package drag-stuff))

(unless (scimax-package-excluded-p 'swiper)
  (use-package swiper
    :bind
    ("C-s" . counsel-grep-or-swiper)
    ("H-s" . swiper-all)
    :diminish ivy-mode
    :config
    (ivy-mode)))

(unless (scimax-package-excluded-p 'counsel)
  (use-package counsel
    :init
    (require 'ivy)
    (setq projectile-completion-system 'ivy)
    (setq ivy-use-virtual-buffers t)
    (define-prefix-command 'counsel-prefix-map)
    (global-set-key (kbd "H-c") 'counsel-prefix-map)

    ;; default pattern ignores order.
    (setf (cdr (assoc t ivy-re-builders-alist))
	  'ivy--regex-ignore-order)
    :bind
    (("M-x" . counsel-M-x)
     ("C-x b" . ivy-switch-buffer)
     ("C-x C-f" . counsel-find-file)
     ("C-x l" . counsel-locate)
     ("C-h f" . counsel-describe-function)
     ("C-h v" . counsel-describe-variable)
     ("C-h i" . counsel-info-lookup-symbol)
     ("H-c r" . ivy-resume)
     ("H-c l" . counsel-load-library)
     ("H-c f" . counsel-git)
     ("H-c g" . counsel-git-grep)
     ("H-c a" . counsel-ag)
     ("H-c p" . counsel-pt))
    :diminish ""
    :config
    (progn
      (counsel-mode)
      (define-key minibuffer-local-map (kbd "C-r") 'counsel-minibuffer-history)
      (define-key ivy-minibuffer-map (kbd "M-<SPC>") 'ivy-dispatching-done)

      ;; C-RET call and go to next
      (define-key ivy-minibuffer-map (kbd "C-<return>")
	(lambda ()
	  "Apply action and move to next/previous candidate."
	  (interactive)
	  (ivy-call)
	  (ivy-next-line)))

      ;; M-RET calls action on all candidates to end.
      (define-key ivy-minibuffer-map (kbd "M-<return>")
	(lambda ()
	  "Apply default action to all candidates."
	  (interactive)
	  (ivy-beginning-of-buffer)
	  (loop for i from 0 to (- ivy--length 1)
		do
		(ivy-call)
		(ivy-next-line)
		(ivy--exhibit))
	  (exit-minibuffer)))

      ;; s-RET to quit
      (define-key ivy-minibuffer-map (kbd "s-<return>")
	(lambda ()
	  "Exit with no action."
	  (interactive)
	  (ivy-exit-with-action
	   (lambda (x) nil))))

      ;; Show keys
      (define-key ivy-minibuffer-map (kbd "?")
	(lambda ()
	  (interactive)
	  (describe-keymap ivy-minibuffer-map)))

      (define-key ivy-minibuffer-map (kbd "<left>") 'ivy-backward-delete-char)
      (define-key ivy-minibuffer-map (kbd "<right>") 'ivy-alt-done)
      (define-key ivy-minibuffer-map (kbd "C-d") 'ivy-backward-delete-char))))

;; Provides functions for working on lists
(unless (scimax-package-excluded-p 'dash)
  (use-package dash))
(unless (scimax-package-excluded-p 'dash-functional)
  (use-package dash-functional))

(unless (scimax-package-excluded-p 'dashboard)
  (use-package dashboard))

(unless (scimax-package-excluded-p 'elfeed)
  (use-package elfeed))

;; Python editing mode
(unless (scimax-package-excluded-p 'elpy)
  (use-package elpy
    :config
    (elpy-enable)))

(unless (scimax-package-excluded-p 'esup)
  (use-package esup))

;; Provides functions for working with files
(unless (scimax-package-excluded-p 'f)
  (use-package f))

;; https://github.com/amperser/proselint
;; pip install proselint
(unless (scimax-package-excluded-p 'flycheck)
  (use-package flycheck
    ;; Jun 28 - I like this idea, but sometimes this is too slow.
    :config
    (add-hook 'text-mode-hook #'flycheck-mode)
    (add-hook 'org-mode-hook #'flycheck-mode)
    (define-key flycheck-mode-map (kbd "s-;") 'flycheck-previous-error)))


;; https://manuel-uberti.github.io/emacs/2016/06/06/spellchecksetup/
(unless (scimax-package-excluded-p 'flyspell-correct-ivy)
  (use-package flyspell-correct-ivy
    :ensure t
    :init
    (if (file-directory-p (expand-file-name "emacs-win" scimax-dir))
	(progn
	  ;; spell-checking on windows
	  (setq ispell-program-name
		(expand-file-name
		 "emacs-win/bin/hunspell"
		 scimax-dir))

	  (setq ispell-dictionary "english")

	  (setq ispell-local-dictionary-alist
		`(("english"
		   "[[:alpha:]]"
		   "[^[:alpha:]]"
		   "[']"
		   t
		   ("-d" "en_US" "-p" ,(expand-file-name
					"emacs-win/share/hunspell/en_US"
					scimax-dir))
		   nil
		   utf-8))))
      (setenv "DICPATH" (expand-file-name "~/Library/Spelling"))
      (setq ispell-program-name (executable-find "hunspell")
	    ispell-dictionary "en_US"
	    ispell-local-dictionary "en_US"
	    ispell-local-dictionary-alist
	    `(("english"
	       "[[:alpha:]]"
	       "[^[:alpha:]]"
	       "[']"
	       t
	       ("-d" "en_US" "-p" ,(expand-file-name "~/Library/Spelling/"))
	       nil
	       utf-8)
	      ("en_US"
	       "[[:alpha:]]"
	       "[^[:alpha:]]"
	       "[']"
	       t
	       ("-d" "en_US" "-p" ,(expand-file-name "~/Library/Spelling/"))
	       nil
	       utf-8))
	    flyspell-correct-interface 'flyspell-correct-ivy))
    (add-hook 'flyspell-incorrect-hook
	      (lambda (beg end sym)
		(message "%s misspelled. Type %s to fix it."
			 (buffer-substring beg end)
			 (substitute-command-keys
			  "\\[flyspell-correct-previous-word-generic]"))
		;; return nil so word is still highlighted.
		nil))
    (add-hook 'text-mode-hook
	      (lambda ()
		(flyspell-mode)
		(flycheck-mode)))

    (add-hook 'org-mode-hook
	      (lambda ()
		(flyspell-mode +1)
		(flycheck-mode +1)))

    :after flyspell
    :config
    (progn
      (define-key flyspell-mode-map (kbd "C-;") 'flyspell-correct-previous-word-generic))))


(unless (scimax-package-excluded-p 'flx)
  (use-package flx))

(unless (scimax-package-excluded-p 'git-messenger)
  (use-package git-messenger
    :bind ("C-x v o" . git-messenger:popup-message)))

;; google-this
(unless (scimax-package-excluded-p 'google-this)
  (use-package google-this
    :config
    (google-this-mode 1)))

(unless (scimax-package-excluded-p 'helm)
  (use-package helm
    :init (setq helm-command-prefix-key "C-c h")
    :bind
    ("<f7>" . helm-recentf)
    ;; ("M-x" . helm-M-x)
    ;; ("M-y" . helm-show-kill-ring)
    ;; ("C-x b" . helm-mini)
    ;; ("C-x C-f" . helm-find-files)
    ;; ("C-h C-f" . helm-apropos)
    :config
    (add-hook 'helm-find-files-before-init-hook
	      (lambda ()
		(helm-add-action-to-source
		 "Insert path"
		 (lambda (target)
		   (insert (file-relative-name target)))
		 helm-source-find-files)

		(helm-add-action-to-source
		 "Insert absolute path"
		 (lambda (target)
		   (insert (expand-file-name target)))
		 helm-source-find-files)

		(helm-add-action-to-source
		 "Attach file to email"
		 (lambda (candidate)
		   (mml-attach-file candidate))
		 helm-source-find-files)

		(helm-add-action-to-source
		 "Make directory"
		 (lambda (target)
		   (make-directory target))
		 helm-source-find-files)))))


(unless (scimax-package-excluded-p 'helm-bibtex)
  (use-package helm-bibtex))

(unless (scimax-package-excluded-p 'helm-projectile)
  (use-package helm-projectile))

(unless (scimax-package-excluded-p 'help-fns+)
  (use-package help-fns+
    :load-path scimax-dir))

;; Functions for working with hash tables
(unless (scimax-package-excluded-p 'ht)
  (use-package ht))

(unless (scimax-package-excluded-p 'htmlize)
  (use-package htmlize))

(unless (scimax-package-excluded-p 'hy-mode)
  (use-package hy-mode))

(unless (scimax-package-excluded-p 'hydra)
  (use-package hydra
    :init
    (setq hydra-is-helpful t)

    :config
    (require 'hydra-ox)))

(unless (scimax-package-excluded-p 'ivy-hydra)
  (use-package ivy-hydra))

(unless (scimax-package-excluded-p 'jedi)
  (use-package jedi))

(unless (scimax-package-excluded-p 'jedi-direx)
  (use-package jedi-direx))

;; Superior lisp editing
(unless (scimax-package-excluded-p 'lispy)
  (use-package lispy
    :config
    (dolist (hook '(emacs-lisp-mode-hook
		    hy-mode-hook))
      (add-hook hook
		(lambda ()
		  (lispy-mode)
		  (eldoc-mode))))))

(unless (scimax-package-excluded-p 'magit)
  (use-package magit
    :init (setq magit-completing-read-function 'ivy-completing-read)
    :bind
    ("<f5>" . magit-status)
    ("C-c v t" . magit-status)))

(unless (scimax-package-excluded-p 'magithub)
  (use-package magithub
    :after magit))

;; Templating system
;; https://github.com/Wilfred/mustache.el
(unless (scimax-package-excluded-p 'mustache)
  (use-package mustache))

;; this is a git submodule
(unless (scimax-package-excluded-p 'ob-ipython)
  (if (executable-find "jupyter")
      (use-package ob-ipython
	:ensure nil
	:load-path (lambda () (expand-file-name "ob-ipython-upstream" scimax-dir))
	:init (add-to-list 'load-path (expand-file-name "ob-ipython-upstream" scimax-dir))
	(require 'ob-ipython))
    (message "jupyter was not found on your path. ob-ipython was not loaded.")))

(unless (scimax-package-excluded-p 'scimax-org-babel-ipython-upstream)
  (use-package scimax-org-babel-ipython-upstream
    :ensure nil
    :load-path scimax-dir))

(unless (scimax-package-excluded-p 'ov)
  (use-package ov))

;; [2019-01-23 Wed] commented out. I don't use this at all, and it causes an error on Windows when starting up.
;; (use-package org-edit-latex)

(unless (scimax-package-excluded-p 'pdf-tools)
  (use-package pdf-tools))

(unless (scimax-package-excluded-p 'org-mime)
  (use-package org-mime
    :ensure nil
    :load-path (lambda () (expand-file-name "org-mime" scimax-dir))
    :init (setq org-mime-up-subtree-heading 'org-back-to-heading
		org-mime-export-options '(:section-numbers nil
						       :with-author nil
						       :with-toc nil
						       :with-latex dvipng))))

;; this is a git submodule
(unless (scimax-package-excluded-p 'org-ref)
  (use-package org-ref
    :ensure nil
    :load-path (lambda () (expand-file-name "org-ref" scimax-dir))
    :init
    (add-to-list 'load-path
		 (expand-file-name "org-ref" scimax-dir))
    (setq bibtex-autokey-year-length 4
	  bibtex-autokey-name-year-separator "-"
	  bibtex-autokey-year-title-separator "-"
	  bibtex-autokey-titleword-separator "-"
	  bibtex-autokey-titlewords 2
	  bibtex-autokey-titlewords-stretch 1
	  bibtex-autokey-titleword-length 5
	  org-ref-bibtex-hydra-key-binding (kbd "H-b"))
    ;; (define-key bibtex-mode-map org-ref-bibtex-hydra-key-binding 'org-ref-bibtex-hydra/body)
    ;; (global-set-key (kbd "H-b") 'org-ref-bibtex-hydra/body)
    ))

(unless (scimax-package-excluded-p 'org-ref-arxiv)
  (use-package org-ref-arxiv
    :ensure nil
    :load-path (lambda () (expand-file-name "org-ref" scimax-dir))))

(unless (scimax-package-excluded-p 'org-ref-scopus)
  (use-package org-ref-scopus
    :ensure nil
    :load-path (lambda () (expand-file-name "org-ref" scimax-dir))))

(unless (scimax-package-excluded-p 'org-ref-wos)
  (use-package org-ref-wos
    :ensure nil
    :load-path (lambda () (expand-file-name "org-ref" scimax-dir))))


;; https://github.com/bbatsov/projectile
(unless (scimax-package-excluded-p 'projectile)
  (use-package projectile
    :init (setq projectile-cache-file
		(expand-file-name "user/projectile.cache" scimax-dir)
		projectile-known-projects-file
		(expand-file-name "user/projectile-bookmarks.eld" scimax-dir))
    :bind
    ("C-c pp" . projectile-switch-project)
    ("C-c pb" . projectile-switch-to-buffer)
    ("C-c pf" . projectile-find-file)
    ("C-c pg" . projectile-grep)
    ("C-c pk" . projectile-kill-buffers)
    ;; nothing good in the modeline to keep.
    :diminish ""
    :config
    (define-key projectile-mode-map (kbd "H-p") 'projectile-command-map)
    (projectile-global-mode)))

(unless (scimax-package-excluded-p 'pydoc)
  (use-package pydoc))

(unless (scimax-package-excluded-p 'rainbow-mode)
  (use-package rainbow-mode))

(unless (scimax-package-excluded-p 'recentf)
  (use-package recentf
    :config
    (setq recentf-exclude
          '("COMMIT_MSG" "COMMIT_EDITMSG" "github.*txt$"
            ".*png$" "\\*message\\*" "auto-save-list\\*"))
    (setq recentf-max-saved-items 60)))


;; Functions for working with strings
(unless (scimax-package-excluded-p 's)
  (use-package s))

(unless (scimax-package-excluded-p 'smart-mode-line)
  (use-package smart-mode-line
    :config
    (setq sml/no-confirm-load-theme t)
    (setq sml/theme 'light)
    (sml/setup)))


;; keep recent commands available in M-x
(unless (scimax-package-excluded-p 'smex)
  (use-package smex))

(unless (scimax-package-excluded-p 'undo-tree)
  (use-package undo-tree
    :diminish undo-tree-mode
    :config (global-undo-tree-mode)))

;; Note ws-butler-global-mode causes some issue with org-ref ref links. If you
;; are right after one you cannot add a space without getting a new line.
(unless (scimax-package-excluded-p 'ws-butler)
  (use-package ws-butler))

(unless (scimax-package-excluded-p 'yasnippet)
  (use-package yasnippet))

(unless (or (scimax-package-excluded-p 'ivy-yasnippet)
            (version-list-<= (version-to-list emacs-version) '(25 3 1)))
  (use-package ivy-yasnippet
    :bind ("H-," . ivy-yasnippet)))

;; * Scimax packages
(use-package scimax
  :ensure nil
  :load-path scimax-dir
  :init (require 'scimax)
  :bind
  ("C-x C-b" . ibuffer))

(use-package scimax-mode
  :ensure nil
  :load-path scimax-dir
  :init (require 'scimax-mode)
  :config (scimax-mode))

(unless (scimax-package-excluded-p 'scimax-org)
  (use-package scimax-org
    :ensure nil
    :load-path scimax-dir
    :bind
    ("s--" . org-subscript-region-or-point)
    ("s-=" . org-superscript-region-or-point)
    ("s-i" . org-italics-region-or-point)
    ("s-b" . org-bold-region-or-point)
    ("s-v" . org-verbatim-region-or-point)
    ("s-c" . org-code-region-or-point)
    ("s-u" . org-underline-region-or-point)
    ("s-+" . org-strikethrough-region-or-point)
    ("s-4" . org-latex-math-region-or-point)
    ("s-e" . ivy-insert-org-entity)
    :init
    (require 'scimax-org)))

(unless (scimax-package-excluded-p 'ox-clip)
  (use-package ox-clip
    :ensure nil
    :load-path (lambda () (expand-file-name "ox-clip" scimax-dir))
    :bind ("H-k" . ox-clip-formatted-copy)))

(unless (scimax-package-excluded-p 'scimax-email)
  (use-package scimax-email
    :ensure nil
    :load-path scimax-dir))

(unless (scimax-package-excluded-p 'scimax-notebook)
  (org-babel-load-file (expand-file-name "scimax-notebook.org" scimax-dir)))
;; (use-package scimax-notebook
;;   :ensure nil
;;   :load-path scimax-dir)

(unless (scimax-package-excluded-p 'scimax-utils)
  (use-package scimax-utils
    :ensure nil
    :load-path scimax-dir
    :bind ( "<f9>" . hotspots)))

(unless (scimax-package-excluded-p 'bibtex-hotkeys)
  (use-package bibtex-hotkeys
    :ensure nil
    :load-path scimax-dir))

(unless (scimax-package-excluded-p 'ox-manuscript)
  (use-package ox-manuscript
    :ensure nil
    :load-path (lambda () (expand-file-name "ox-manuscript" scimax-dir))))

(unless (scimax-package-excluded-p 'org-show)
  (use-package org-show
    :ensure nil
    :load-path (lambda () (expand-file-name "org-show" scimax-dir))))

;; (use-package techela
;;   :ensure nil
;;   :load-path (lambda () (expand-file-name "techela" scimax-dir)))

(unless (scimax-package-excluded-p 'words)
  (use-package words
    :ensure nil
    :load-path scimax-dir
    :bind ("H-w" . words-hydra/body)))

(unless (scimax-package-excluded-p 'ore)
  (use-package ore
    :ensure nil
    :load-path scimax-dir
    :bind ("H-o" . ore)))

;; (use-package org-editmarks
;;   :ensure nil
;;   :load-path scimax-dir)

(unless (scimax-package-excluded-p 'scimax-ivy)
  (use-package scimax-ivy
    :ensure nil
    :load-path scimax-dir))

(unless (scimax-package-excluded-p 'scimax-lob)
  (use-package scimax-lob
    :ensure nil
    :load-path scimax-dir))

(unless (scimax-package-excluded-p 'scimax-yas)
  (use-package scimax-yas
    :ensure nil
    :load-path scimax-dir))

(unless (scimax-package-excluded-p 'scimax-autoformat-abbrev)
  (use-package scimax-autoformat-abbrev
    :ensure nil
    :load-path scimax-dir))

(unless (scimax-package-excluded-p 'scimax-hydra)
  (use-package scimax-hydra
    :ensure nil
    :load-path scimax-dir
    :bind ("<f12>" . scimax/body)))

(unless (scimax-package-excluded-p 'scimax-journal)
  (use-package scimax-journal
    :ensure nil
    :load-path scimax-dir))

(unless (scimax-package-excluded-p 'scimax-apps)
  (use-package scimax-apps
    :ensure nil
    :load-path scimax-dir))

(unless (scimax-package-excluded-p 'scimax-ob)
  (use-package scimax-ob
    :ensure nil
    :load-path scimax-dir))

(unless (scimax-package-excluded-p 'kitchingroup)
  (use-package kitchingroup
    :ensure nil
    :load-path scimax-dir))

;; (use-package ov-highlight
;;   :ensure nil
;;   :load-path (lambda () (expand-file-name "ov-highlight" scimax-dir))
;;   :bind ("H-h" . ov-highlight/body)
;;   :init
;;   (add-to-list 'load-path
;; 	       (expand-file-name "ov-highlight" scimax-dir))
;;   (require 'ov-highlight))

(unless (scimax-package-excluded-p 'scimax-editmarks)
  (org-babel-load-file (expand-file-name "scimax-editmarks.org" scimax-dir)))

;; * User packages

;; We load one file: user.el

(when (and
       scimax-load-user-dir
       (file-exists-p (expand-file-name "user.el" scimax-user-dir)))
  (load (expand-file-name "user.el" scimax-user-dir)))

(add-to-list 'Info-directory-list scimax-dir)

;; * The end
(provide 'packages)

;;; packages.el ends here
